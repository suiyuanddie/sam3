...
try:
    ...
    # PyTorch < 2.0 compatibility: simple fallback for tree_map_only
    def tree_map_only(ty, fn, tree):
        if isinstance(tree, ty):
            return fn(tree)
        elif isinstance(tree, (list, tuple)):
            return type(tree)(tree_map_only(ty, fn, x) for x in tree)
        elif isinstance(tree, dict):
            return {k: tree_map_only(ty, fn, v) for k, v in tree.items()}
        return tree
